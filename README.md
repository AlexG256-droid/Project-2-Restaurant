...
We used Claude AI and ChatGPT to help with the JS code that fits with the Favorites, Restaurants, and Random pages. While we setup the basic HTML and JS structure for those specific pages, the model helped generate some of the JS code needed for the website itself to function properly so that the user can pick and/or add his or her desired restaurants into a specified folder given specific restaurant data that was already added. The model type that Alex used was Claude Opus 4.8. These were the following prompts for the model:
